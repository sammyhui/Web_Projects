...
if (!isset($_GET['id_sewa_transaksi']) || !is_numeric($_GET['id_sewa_transaksi'])) {
    echo "Transaksi tidak valid.";
    exit;
}

$id_transaksi = (int) $_GET['id_sewa_transaksi'];

// Ambil transaksi dan info metode pembayaran
$sql = "SELECT st.*, s.metode_dana, s.metode_ovo, s.metode_bank 
        FROM sewa_transaksi st
        JOIN tb_sewa s ON st.id_sewa = s.id
        WHERE st.id = $id_transaksi";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_FILES['bukti'])) {
    ...
    $newFileName = uniqid('bukti_sewa_', true) . '.' . $fileExt;
    $newTargetFile = $uploadDir . $newFileName;

    if (move_uploaded_file($fileTmp, $newTargetFile)) {
        $update = mysqli_query($conn, "UPDATE sewa_transaksi SET bukti_tf='$newFileName', status='selesai', tanggal_upload_bukti=NOW() WHERE id=$id_transaksi");
        if ($update) {
            echo "<script>
                    alert('Pesanan sewa selesai! Terima kasih.');
...
<head>
    <meta charset="UTF-8" />
    <title>Upload Bukti Transfer Sewa</title>
    <style>
        body {
...
<body>
    <div class="container">
        <h2>Upload Bukti Transfer Sewa</h2>
